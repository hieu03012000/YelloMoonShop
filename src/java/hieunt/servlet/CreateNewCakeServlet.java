    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (!isMutilPart) {
                ...
                boolean err = false;
                String name = "";
                if (params.get("txtNewName") != null) {
                    name = (String) params.get("txtNewName");
                }
                ...
                String p = "";
                float price = 0;
                if (params.get("txtNewPrice") != null) {
                    p = (String) params.get("txtNewPrice");
                }
                try {
                ...
                String q = "";
                int quantity = 0;
                if (params.get("txtNewQuantity") != null) {
                    q = (String) params.get("txtNewQuantity");
                }
                try {
                ...
                String c = "";
                if (params.get("txtNewCDate") != null) {
                    c = (String) params.get("txtNewCDate");
                }
                String e = "";
                if (params.get("txtNewEDate") != null) {
                    e = (String) params.get("txtNewEDate");
                }
                if (c.equals("") || e.equals("")) {
                    errors.setcDateError("Choose a date");
                    err = true;
                } else {
                ...
                String category = "";
                if (params.get("cbNewCategory") != null) {
                    category = (String) params.get("cbNewCategory");
                }
                HttpSession session = request.getSession();

                if (!err) {
                    ProductDTO pDTO = new ProductDTO("", name, price, quantity, image, c, e, category, true, description);
                    pDAO.addNewProduct(pDTO);
                    session.removeAttribute("ERROR");
                    LogDAO lDAO = new LogDAO();
                    UserDTO uDTO = (UserDTO) session.getAttribute("USER_DTO");
                    String productID = pDAO.getNewestProductID(name);
                    lDAO.addLog(new LogDTO("", uDTO.getEmail(), productID, "", "create"));
                } else {
                    session.setAttribute("ERROR", "Can not create");
                }
